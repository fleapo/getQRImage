...
def run(words, version=1, level='H', picture=None, colorized=False, contrast=1.0, brightness=1.0, save_name=None, save_dir=os.getcwd(), content_only=False):
    ...
    try:
        if (not content_only) and (not os.path.exists(tempdir)):
            os.makedirs(tempdir)

        ver, qr_name = theqrmodule.get_qrcode(version, level, words, tempdir, content_only)
        if content_only:
            return qr_name
